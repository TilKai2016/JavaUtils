...
/**
 * Crc16校验(这只是Crc16校验的一种实现，实际存在多种Crc值生成策略)
 * @author tilkai
 */
public class Crc16Utils {
    ...
    /**
     * 获取crc16校验结果
     * @param data 待比较的带有Crc校验值的byte数组
     * @return true || false
     */
    public static boolean CRC16Checked (byte[] data) {

        byte[] dataExceptCrc = new byte[data.length - 2]; // 去除CRC校验码的data字节数组

        System.arraycopy(data, 0, dataExceptCrc, 0, data.length-2);

        int crc = calcCrc16(dataExceptCrc);
        String sFormat = String.format("%04x", crc);
        ...
        if (sFormat.length() == 4) {
            crc1 = (byte) Integer.parseInt(sFormat.substring(2, 4), 16);
            crc2 = (byte) Integer.parseInt(sFormat.substring(0, 2), 16);
        } else {
            crc1 = (byte) Integer.parseInt(sFormat.substring(1, 3), 16);
            crc2 = (byte) Integer.parseInt(sFormat.substring(0, 1), 16);
        }

        return crc1 == data[data.length - 2] && crc2 == data[data.length - 1];
    }

    /**
     * 获取给定byte数组的Crc校验值
     * @param dataExceptCrc 不带有Crc校验值的byte数组
     * @return {crc1, crc2}
     */
    public static byte[] getCRC16Array (byte[] dataExceptCrc) {
        ...
        return new byte[] {crc1, crc2};
    }

    public static void main(String []args) {

        boolean cfg = CRC16Checked(new byte[] {
                0x69, (byte) 0xbb, (byte) 0xcc, 0x0a, 0x58, (byte) 0xdb, 0x22, (byte) 0x96, (byte) 0xff});
